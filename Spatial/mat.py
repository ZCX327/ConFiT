...
class Discriminator(torch.nn.Module):
    ...
    def forward(self, images_in, masks_in, images_stg1, c):
        # print("input.shape",images_in.shape)
        # print("mask.shape",masks_in.shape)
        # print("images_stg1.shape",images_stg1.shape)


        # 假设 `images_stg1` 是 RGB（3通道）图像，`input_image` 是 RGBA（4通道）图像
        if images_stg1.shape[1] == 3 and images_in.shape[1] == 4:
            # 提取 `input_image` 的 Alpha 通道（即第 4 通道）
            alpha_channel = images_in[:, 3:4, :, :]  # 获取 alpha 通道，形状为 [batch_size, 1, height, width]
            
            # 将 `alpha_channel` 添加到 `images_stg1` 后面，得到 RGBA 格式的 `images_stg1`
            images_stg1 = torch.cat([images_stg1, alpha_channel], dim=1)  # 新的 images_stg1 是 4 通道（RGBA）
            
        # print("masks_in shape:", masks_in.shape)
        # print("images_stg1 shape:", images_stg1.shape)
        # print("concat shape1:", torch.cat([masks_in - 0.5, images_in], dim=1).shape)
        ...
        # print("concat shape2:", torch.cat([masks_in - 0.5, images_stg1], dim=1).shape)
