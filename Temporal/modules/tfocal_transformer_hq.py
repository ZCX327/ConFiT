...
class SoftSplit(nn.Module):
    ...
    def forward(self, x, b, output_size):
        ...
        # print(f"t2t: {self.t2t}")
        # print(f"t2t_param: {self.t2t_param}")
        ...
        # print(f"f_h: {f_h}, f_w: {f_w}")

        feat = self.t2t(x)
        # print(f"feat shape after unfold: {feat.shape}")
        feat = feat.permute(0, 2, 1)
        # print(f"feat shape after permute: {feat.shape}")
        # feat shape [b*t, num_vec, ks*ks*c]
        feat = self.embedding(feat)
        # print(f"feat shape after embedding: {feat.shape}")
        # feat shape after embedding [b, t*num_vec, hidden]
        feat = feat.view(b, -1, f_h, f_w, feat.size(2))
        # print(f"feat shape after view: {feat.shape}")
        return feat
...
def window_partition(x, window_size):
    ...
    """
    B, T, H, W, C = x.shape
    # print(f"Input tensor shape: {x.shape}")  # 输出输入张量的形状
    
        
    if H % window_size[0] != 0 or W % window_size[1] != 0:
        raise ValueError(f"Height and Width must be divisible by the window size.")
