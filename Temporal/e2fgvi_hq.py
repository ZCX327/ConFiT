...
class InpaintGenerator(BaseNetwork):
    ...
    def pad_to_540_btchw(self,x):
        ...
        # F.pad 支持 (N,C,H,W)，先合并 B*T
        x = x.view(B*T, C, H, W)
        x_pad = F.pad(x, (pad_left, pad_right, pad_top, pad_bottom), mode='constant', value=0)
        x_pad = x_pad.view(B, T, C, 540, 540)
        return x_pad, (pad_top, pad_bottom, pad_left, pad_right)
    
    # 假设 output 是 (B*T, C, H, W)
    def remove_pad_540_bchw(self,output, pad_info):
        pad_top, pad_bottom, pad_left, pad_right = pad_info
        return output[:, :, pad_top:output.size(2)-pad_bottom, pad_left:output.size(3)-pad_right]




    def forward(self, masked_frames, num_local_frames):
        masked_frames, pad_info = self.pad_to_540_btchw(masked_frames)
        ...
        zjz = enc_feat.view(-1, c, h, w)
        trans_feat = self.ss(zjz , b, fold_output_size)
        ...
        output = self.remove_pad_540_bchw(output, pad_info)
